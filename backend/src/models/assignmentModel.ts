...
export const getAssignmentForLogsQuery = async (task_id: string, user_id: string) => {
    const result = await pool.query(`
        SELECT 
            assignments.*, 
            users.name AS user_name, 
        ...
        WHERE assignments.task_id = $1 AND assignments.user_id = $2;`, [task_id, user_id]); // send a query to the database with one of the open connection from the pool
    return result.rows
}
