...
export const ProjectLogsComponent = ({ projectId }: ProjectLogProps) => {
        ...
        switch (log.table_name) {
            case 'assignments':
                if (log.operation.toLowerCase() === 'insert') {
                    if (itemName === "you") {
                        return (
                            <>
                                You assigned to task {renderTaskLink(log)}
                            </>
                        );
                    }
                    return (
                        <>
                            Assigned {itemName} to task {renderTaskLink(log)}
                        </>
                    );
                    ...
                    if (itemName === "you") {
                        return (
                            <>
                                You unassigned from task {renderTaskLink(log)}
                            </>
                        );
                    }
                    return (
                        <>
                            Unassigned {itemName} from task {renderTaskLink(log)}
                        </>
                    );
                }
            ...
            case 'pending_project_invitations':
                if (log.operation.toLowerCase() === 'insert') {
                    ...
                    return (
                        <>
                            Cancelled invitation to {itemName}
                        </>
                    );
                }
            ...
            case 'comments':
                if (log.operation.toLowerCase() === 'insert') {
                    return (
                        <>
                            Wrote a comment to task {renderTaskLink(log)}
                        </>
                    );
                    ...
                    if (changedFields.length === 0) {
                        return (
                            <>
                                Updated a comment on task {renderTaskLink(log)}
                            </>
                        );
                    }
                    ...
                    return (
                        <div className="space-y-1">
                            <span>
                                Updated comment on task {renderTaskLink(log)} :
                            </span>
                    ...
                    return (
                        <>
                            Deleted a comment from task {renderTaskLink(log)}
                        </>
                    );
                }
                return `Modified comment in project`;
            case 'milestones':
                if (log.operation.toLowerCase() === 'insert') {
                    return (
                        <>
                            Created milestone {renderMilestoneLink(log)}
                        </>
                    );
                } else if (log.operation.toLowerCase() === 'update') {
                    const changedFields = [];
                    ...
                    if (changedFields.length === 0) {
                        return (
                            <>
                                Updated milestone {renderMilestoneLink(log)}
                            </>
                        );
                    }
                ...
                return (
                    <>
                        Modified milestone {renderMilestoneLink(log)}
                    </>
                );
            case 'files':
                ...
                if (log.operation.toLowerCase() === 'insert') {
                    return (
                        <>
                            Uploaded file {fileLink} to task {taskLink}
                        </>
                    );
                } else if (log.operation.toLowerCase() === 'update') {
                    return (
                        <>
                            Updated file {fileLink} to task {taskLink}
                        </>
                    );
                } else if (log.operation.toLowerCase() === 'delete') {
                    return (
                        <>
                            Deleted file {fileLink} {log.new_data?.task_id || log.old_data?.task_id ? `from task ${taskLink}` : `from this project`}
                        </>
                    );
                }
            ...
            case 'project_members':
                if (log.operation.toLowerCase() === 'insert') {
                        ...
                        return (
                            <>
                                {log.projectMember.inviter_user_id === authState.user.id
                                    ? "You invited"
                                    : `${log.projectMember.inviter_user_name} invited`} {log.projectMember.user_name} ({log.projectMember.user_email}) to this project
                            </>
                        )
                    }
                } else if (log.operation.toLowerCase() === 'delete') {
                    return (
                        <>
                            Removed {log.projectMember.user_name} ({log.projectMember.user_email})
                        </>
                    )
                } else if (log.operation.toLowerCase() === 'update') {
                    const changedFields = [];
                    ...
                    if (changedFields.length === 0) {
                        return (
                            <>
                                Modified {log.projectMember.user_name}'s membership
                            </>
                        );
                    }
                    ...
                    return (
                        <div className="space-y-1">
                            <span>
                                Updated {log.projectMember.user_name}'s membership :
                            </span>
                            <ul className="list-disc list-inside space-y-1 pl-2 text-sm">
                                {changedFields.map((change, index) => (
                                    <li key={index} className="flex flex-wrap items-baseline">
            ...
            case 'task_labels':
                ...
                if (log.operation.toLowerCase() === 'insert') {
                    return (
                        <>
                            Added label {renderLabelLink(log)} to task {taskLinkTL}
                        </>
                    );
                } else if (log.operation.toLowerCase() === 'delete') {
                    return (
                        <>
                            Removed label {renderLabelLink(log)} from task {taskLinkTL}
                        </>
                    );
                }
                return (
                    <>
                        Modified label {renderLabelLink(log)} on task {taskLinkTL}
                    </>
                );
            case 'projects':
                if (log.operation.toLowerCase() === 'insert') {
                ...
                if (log.operation.toLowerCase() === 'insert') {
                    ...
                    if (log.task?.parent_task_id) {
                        return (
                            <div className="space-y-1">
                                <div>
                                    Added subtask {renderTaskLink(log)} to parent task project
                                </div>
                    ...
                    return (
                        <>
                            Added task {renderTaskLink(log)} to project
                        </>
                    );
                } else if (log.operation.toLowerCase() === 'update') {
                    const changedFields = [];
                    ...
                    if (changedFields.length === 0) {
                        if (log.task?.parent_task_id) {
                            return (
                                <div className="space-y-1">
                                    <div>
                                        Modified subtask {renderTaskLink(log)}
                                    </div>
                                    {renderParentTaskConnection(log)}
                                </div>
                            )
                            ...
                            return (
                                <>
                                    Modified task {renderTaskLink(log)}
                                </>
                            );
                        }
                    }
                    ...
                    if (log.task?.parent_task_id) {
                        return (
                            <div className="space-y-2">
                                <div className="space-y-1">
                                    <span>
                                        Updated subtask {renderTaskLink(log)} :
                                    </span>
                                    <ul className="list-disc list-inside space-y-1 pl-2 text-sm">
                                        {changedFields.map((change, index) => (
                                            <li key={index} className="flex flex-wrap items-baseline">
                        ...
                        return (
                            <div className="space-y-1">
                                <span>
                                    Updated task {renderTaskLink(log)} :
                                </span>
                                <ul className="list-disc list-inside space-y-1 pl-2 text-sm">
                                    {changedFields.map((change, index) => (
                                        <li key={index} className="flex flex-wrap items-baseline">
    ...
    return (
        <div className="space-y-4">
            {logs.map((log) => (
                <div
                    key={log.id}
                    className="flex gap-4 p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:shadow-sm transition-all duration-200 hover:border-indigo-100 dark:hover:border-indigo-900/50"
                >
                    <div className="flex-shrink-0">
                        <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${getOperationColor(log.table_name)} shadow-xs`}>
